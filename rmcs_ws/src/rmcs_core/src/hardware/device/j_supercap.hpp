...
class Jsupercap {
public:
    explicit Jsupercap(Component& status_component, double max_capacity_voltage = 23.0)
    ...
    void store_status(std::span<const std::byte> can_data) {
        ...
        can_data_.store(std::bit_cast<SupercapStatus>(packet.data), std::memory_order::relaxed);
        supercap_watchdog_.reset(1'000);
        is_data_valid_ = true;
    }
